...
void pull(int now) {  // update now with 2 children
    ...
    // tree[now].middle_max = max(max(tree[lp].middle_max, tree[rp].middle_max),
    // tree[lp].suffix+tree[rp].prefix); tree[now].middle_max =
    // max(max(tree[now].middle_max, tree[now].prefix), tree[now].suffix);
}
// Lazy
