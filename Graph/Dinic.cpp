...
void build() {
    rep(i, 1, m) {
        ...
        adj[v].push_back(
            {u, 0, (int)adj[u].sz - 1});  // have pushed one，need to -1
    }
}
// Police Chase，need to open adj to Augment && ori to determine what pb give
// Dinic、dfs2，then use reach as u，if the edge pb has given && w == 0 && v is
// not in reach，is the ans
...
// Distinct Route
// edge set valid var，if we need to argument pos road，the reverse edge set
// true valid； if we need argument the argumented edge，both set false，last,
